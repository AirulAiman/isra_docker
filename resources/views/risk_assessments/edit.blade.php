...
<div class="container">
    ...
    <form action="{{ route('risk_assessments.update', $riskAssessment->id) }}" method="POST">
        ...
        <div class="form-group mb-3">
          ...
          {{--  Control Group and existing control --}}
          <div class="mb-3">
            <label for="control_group_id" class="form-label">Control Group</label>
            <select id="control_group_id" name="control_group_id" class="form-control" required>
                <option value="" disabled selected>Select a Control Group</option>
                @foreach($controlGroups as $group)
                    <option value="{{ $group->id }}" {{ $riskAssessment->control_group_id == $group->id ? 'selected' : '' }}>
                        {{ $group->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            <label for="existing_control_id" class="form-label">Existing Control</label>
            <select id="existing_control_id" name="existing_control_id" class="form-control" required>
                <option value="" disabled selected>Select an Existing Control</option>
                @foreach($controlGroups as $group)
                    @foreach($group->controls as $control)
                        <option value="{{ $control->id }}" 
                                data-group="{{ $group->id }}" 
                                {{ $riskAssessment->existing_control_id == $control->id ? 'selected' : '' }}>
                            {{ $control->name }}
                        </option>
                    @endforeach
                @endforeach
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Function to update scores and display values
        function updateScoreDisplays() {
            ...
            document.getElementById('cia_impact_score_display').value = ciaImpactScore;
            document.getElementById('impact_level_display').value = impactLevel;
            document.getElementById('final_risk_level_display').value = finalRiskLevel;
        }
    
        // Add event listeners for score updates
        const scoreInputs = ['confidentiality', 'integrity', 'availability', 'business_loss', 'likelihood'];
        scoreInputs.forEach(id => {
            document.getElementById(id).addEventListener('change', updateScoreDisplays);
        });
    
        // Function to dynamically filter Existing Controls based on selected Control Group
        const controlGroupSelect = document.getElementById('control_group_id');
        const existingControlSelect = document.getElementById('existing_control_id');
    
        function filterExistingControls() {
            const selectedGroup = controlGroupSelect.value;
            Array.from(existingControlSelect.options).forEach(option => {
                if (option.dataset.group === selectedGroup || option.value === '') {
                    option.hidden = false;
                } else {
                    option.hidden = true;
                }
            });
            existingControlSelect.value = ''; // Reset the existing control selection
        }
    
        // Add event listener for Control Group selection changes
        controlGroupSelect.addEventListener('change', filterExistingControls);
    
        // Initial filter based on current Control Group selection
        filterExistingControls();
    });
    </script>
